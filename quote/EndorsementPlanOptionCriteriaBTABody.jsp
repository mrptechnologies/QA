...
<html:form action="/EndorsementRate.do" onsubmit="return validateCoverages(this);">
		...
		<tr>
			...
			if(subquoteid !=0 && subquoteid!=-1){ %>
				...
				<%} %>
					...
					<%String pquoteId="";%>
			<%if (request.getAttribute("QuoteId")!= null) {%>
			<%pquoteId= request.getAttribute("QuoteId").toString();%>
			<html:hidden property="answer(Object::Quote::ParentQuoteId)" value="<%=pquoteId%>" />
			<html:hidden property="answer(Object::UWWorkSheet::ParentQuoteId)" value="<%=pquoteId%>" />
				
			<%}else{%>		
			<bean:define id="ParentQuoteId1" property="answer(Object::Quote::ParentQuoteId)" name="AddParentQuote"></bean:define>
			<html:hidden property="answer(Object::Quote::ParentQuoteId)" value="<%=""+ParentQuoteId1%>" />
			<html:hidden property="answer(Object::UWWorkSheet::ParentQuoteId)" value="<%=""+ParentQuoteId1%>" />
			<%} %>	
					...
					<logic:equal value="COVERAGES & LIMIT" name="HEADING">
					 <tr>
						...
						<td colspan="2" class="links">	
							<a href="../GetUWQuoteSummaryAction.do?TabPanelName=Quote_Panel&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyIdTemp.toString()%>&answer(QuoteId)=<%=pquoteId.toString()%>&answer(parentQuoteId)=<%=pquoteId.toString()%>&answer(policyNumber)=<%=""+policyNumber %>&answer(Object::Policy::ParentPolicyId)=<%=""+parentPolicyId %>&answer(Object::Policy::ParentPolicyVerId)=<%=""+parentPolicyVerId %>"  title="<bean:message key='ToolTip.CoveragesLimits.QuoteSummary'/>">Quote Summary</a>
						</td>
					</tr>
...
<div class="formhead"><h2 class="left"><logic:equal value="COVERAGES & LIMIT" name="HEADING">Coverages and Limits</logic:equal><logic:equal value="CANCEL OPTIONS" name="HEADING">Cancel Endorsement</logic:equal></h2></div>  
	...
	<div id='content' >
		...
		<div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'>
			<dmshtml:isFeatureAvailableForProductRoleStatus roleId='<%=Long.parseLong(""+objDescId) %>' productId='<%=Long.parseLong(""+prodId) %>' dbName='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>' featureId="RATE_AVAILABLE" property='<%=new String[]{""}%>' value="" negated="true">
				<div class='navValue' title="<bean:message key='ToolTip.CoveragesLimits.Rate'/>"><INPUT TYPE='submit' value='Rate' style="border:0px;" class="covbutton left" id="RateTop" onclick="return goSubmit(document.AddParentQuote.RateTop,'Rate')" name='answer(form::ForwordControl1)'></div>
	...
	<logic:equal value="COVERAGES & LIMIT" name="HEADING"><div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'><div class='navValue' style="border:0px;" title="<bean:message key='ToolTip.CoveragesLimits.Save'/>"><INPUT TYPE='submit' value='Save' id="SaveTop"  class='covbutton' style="border:0px;" onclick="return goSubmit(document.AddParentQuote.SaveTop,'Save')" name='answer(form::ForwordControl1)'></div></div></logic:equal>
	</div></div>
...
<%String tooltip=""; %>
	...
	<logic:equal value="COVERAGES & LIMIT" name="HEADING">
		<div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'>
		<dmshtml:isFeatureAvailableForProductRoleStatus roleId='<%=Long.parseLong(""+objDescId) %>' productId='<%=Long.parseLong(""+prodId) %>' dbName='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>' featureId="RATE_AVAILABLE" property='<%=new String[]{""}%>' value="" negated="true">
			<div class='navValue' title="<bean:message key='ToolTip.CoveragesLimits.Rate'/>"><INPUT TYPE='submit' value='Rate' class="covbutton left" style="border:0px;" id="RateBottom" onclick="return goSubmit(document.AddParentQuote.RateBottom,'Rate')" name='answer(form::ForwordControl1)'></div>
		</dmshtml:isFeatureAvailableForProductRoleStatus>
	...
	<div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'><div class='navValue' style="border:0px;" title="<bean:message key='<%=tooltip%>'/>"><INPUT TYPE='submit' value='Save' id="SaveBottom" class="covbutton left" style="border:0px;" onclick="return goSubmit(document.AddParentQuote.SaveBottom,'Save')" name='answer(form::ForwordControl1)'></div></div>
	</div>
...
<html:hidden property="dirty" styleId="dirty" value="N" />
	...
	<%if (request.getAttribute("QuoteId")!= null) {%>
			...
			<%}else{%>
				<bean:define id="ParentQuoteId2" property="answer(Object::Quote::ParentQuoteId)" name="AddParentQuote"></bean:define>
			<html:hidden property="answer(Object::Quote::ParentQuoteId)" value="<%=ParentQuoteId2.toString()%>" />
			<html:hidden property="answer(Object::UWWorkSheet::ParentQuoteId)" value="<%=ParentQuoteId2.toString()%>" />
			<%} %>		
