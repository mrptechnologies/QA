...
%>
	...
	<bean:define id="renewingPolicy" name="policyMap" property="Object::Policy::PolicyNumber"/>
	...
	<bean:define id="renewingPolicyEffectiveDate" name="policyMap" property="Object::Policy::PolEfftDate"/>
	<bean:define id="renewingPolicyExpirationDate" name="policyMap" property="Object::Policy::PolExptDate"/>
	...
	<bean:define id="renewingPolicyTotalPremium" name="policyMap" property="Object::Policy::TotalPremium"/>	
	...
	<%--<dmshtml:Get_Customer_Agency_Agent_Details nameInSession="customerDetail1" customerId="<%=custId+""%>" dbname='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>'/> 
   	<logic:present name="customerDetail1" scope="request"> 
		...
		{"SNo":"<%=sNo+"" %>","RenewalQuote":"<%=renewalQuote+"" %>","RenewalQuoteNumber":"<%=renewalQuoteNumber+"" %>","InsuredName":"<%=""+insuredName%> ","AgencyName":"<bean:write name="policyMap" property="Object::Policy::AgencyName"/>","RenewalQuoteStatus":"<%=renewalquoteStatus+"" %>",
		"RenewalPremium":"<%=(renewalPremium+"")%>","RenewalTerm":"<%=(renewalTerm+"")%>","RenewalQuoteEffectiveDate":"<%=(renewalQuoteEffectiveDate+"").substring(0, 10) %>","RenewalQuoteCreatedDate":"<%=(renewalQuoteCreatedDate+"").substring(0, 10) %>","RenewalQuoteCreatedBy":"<%=renewalQuoteCreatedBy+"" %>","RenewingPolicy":"<%=renewingPolicy+"" %>",
		"RenewingPolicyEffectiveDate":"<%=(renewingPolicyEffectiveDate+"").substring(0, 10) %>","RenewingPolicyExpirationDate":"<%=(renewingPolicyExpirationDate+"").substring(0, 10) %>","RenewingPolicyTotalPremium":"<%=(renewingPolicyTotalPremium+"") %>","RenewingPolicyProductName":"<bean:write name="policyMap" property="Object::Policy::ProductName"/>",
		"PolicyId":"<%=(policyId+"")%>","PolicyVerId":"<%=(policyVerId+"")%>","PolicyNumberInVer":"<%=(policyNumberInVer+"")%>","PolicyStatus":"<%=(policyStatus+"")%>","ownerId":"<%=ownerId1+"" %>","userId1":"<%=(userId+"")%>","agencyId":"<bean:write name="policyMap" property="Object::Policy::AgencyId"/>","CustomerId":"<bean:write name="policyMap" property="Object::Policy::CustomerId" />","totRecords":"<%=policySize+"" %>"}
