				...
				<td>
					...
					<logic:equal value="TRUE" name="paramValue">
						<jsp:include page="../common/QuoteHeader.jsp" >
							...
							<jsp:param name="heading" value="<%="QUOTE/RISK APPLICATION"%>" />	
						</jsp:include>
					</logic:equal>
					</logic:present>
					<logic:present name="paramValue">
...
<tr>
	...
	<logic:present name="paramValue">
		<logic:equal value="FALSE" name="paramValue">
				<td class="links"  width="150px" align="left">
					<a href="../GetRiskListCL.do?TabPanelName=Quote_Panel&answer(TabPanelName)=Quote_Panel&answer(IsApplicationDisabled)=<%=""+IsApplicationDisabled%>&customerId=<%=""+customerId%>&answer(Object::UserDetail::ownerId)=<%=""+ownerId %>&answer(RiskType)=COMMERCIAL&SearchObjectId=<%=""+customerId%>&answer(Object::Customer::CustomerId)=<%=""+customerId%>&answer(Object::Risk::ComProperty::CustomerId)=<%=""+customerId%>&answer(Object::Agency::AgencyId)=<%=""+agencyId%>&answer(objectId)=<%=""+ProductId%>&answer(objectVerId)=<%=""+productVerId%>&answer(objectName)=PRODUCT&answer(applicationType)=RISK&answer(applicationSubType)=&Navigation=&answer(prdGroupId)=<%=""+prdGroupId%>&answer(prdGroupVerId)=<%=""+prdGroupVerId%>&answer(productStatus)=<%=""+productStatus%>&answer(teritory)=<%=""%>&answer(riskTerritory)=<%=""%>&answer(territoryName)=<%=""%>&answer(zone)=<%=""%>&answer(state)=<%=""%>&answer(showOnetTab)=Y&tabNo=0&answer(Object::Employee::PageNum)=0&answer(Object::Customer::State)=<%=""+CustomerState%>&answer(Object::Risk::ComProperty::CustomerId)=<%=""+customerId%>&answer(Object::Risk::ComProperty::ComPropertyId)=-1&answer(SelectedRiskType)=COMMERCIAL<%=" "%>AUTO&answer(parentQuoteId)=<%=parentQuoteId.toString()%>&parentQuoteId=<%=parentQuoteId.toString()%>&answer(subQuoteId)=<%=SubQuoteId.toString()%>&answer(QuoteId)=<%=SubQuoteId.toString()%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyId.toString()%>">Risk List</a>
				</td>	   				
		</logic:equal>
	</logic:present>
			<!-- Commented the Risks Link in Quote Side Risk Application Page as per Siby's Instruction - Abirami DJ
