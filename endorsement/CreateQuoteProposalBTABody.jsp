											...
											<tr>
												<td>
													<jsp:include page="../common/DynamicPolicyHeaderUI3.jsp">
														...
														<jsp:param name="policyNumber" value="<%=""+policyNumber%>" />
														<jsp:param name="prodId" value="<%=""+productId1%>" />
														<jsp:param name="ProductVerId" value="<%=""+ProductVerId%>" />								
												...
												<dmshtml:dms_isPresent property="<%=new String[]{"Sold", "Lost", "Rejected"}%>" value="<%=strParetQuoteStatus.toString()%>" >
													<dmshtml:dms_isPresent property='<%=new String[]{"MANDATORY_RULES_FAILED","ELIGIBILITY_RULES_FAILED","SUBMISSION_RULES_SUCCESS","failure"}%>' value="<%=ruleStatus.toString()%>" negated="false">
														...
														<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"PREVIEW_PROPOSAL_DOCUMENT"}%>' connector='or' negated="true">		
															<INPUT TYPE="BUTTON" VALUE="Preview Proposal Document" class="covbigbutton1" style="width:220px;font-size:13px;" ONCLICK="previewDoc()">
														</dmshtml:dms_static_with_connector>																										
													</dmshtml:dms_isPresent>																 				
												</dmshtml:dms_isPresent>
												<%}%>
...
<script type="text/javascript">
	...
	var pars =  '&answer(Object::Quote::ProductId)='+productId+'&answer(Object::Quote::ProductVerId)='+ProductVerId+'&answer(Object::Quote::ParentQuoteId)='+strParentQuoteId+
				...
				'&answer(Object::Quote::SubQuoteId)='+strSubQuoteId+'&answer(subQuoteId)='+strSubQuoteId+'&answer(isPreview)=true&answer(subQuoteVerId)='+strSubQuoteVerId;
	...
	new Ajax.Request(url, {method: 'post',onComplete:function(transport){},onSuccess: function(transport){
		...
		if(respText == "success"){
			window.open('/PreviewProposalPdf.go?&answer(DocumentType)=PROPOSAL&answer(DocumentListSource)=Quote&answer(QuoteId)='+strParentQuoteId+'&answer(ownerId)='+ownerIdVal+'&answer(subQuoteId)='+strSubQuoteId+'&answer(subQuoteVerId)='+strSubQuoteVerId+'&answer(isPreview)=<%=true%>');
		}else{
