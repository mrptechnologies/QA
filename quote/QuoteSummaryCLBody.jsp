...
<bean:define id="stateCode" name="QuoteDetail" property="answer(Object::Plan::StateCode)" />
	...
	<table WIDTH="100%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
    	...
    	<table WIDTH="100%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
			...
			<tr>
				...
				<logic:present name="ClearanceErrorMessage" scope="request">
					<tr>
						<td class="Error">Clearance Failed.</td>
					</tr>
				</logic:present>
...
<!--########################## POLICY #########################################-->
		...
		<%if((!policyNumber.toString().equals("")) && numberOfSubQuotes==1) {%>
				...
				<td class="links" align="left" width="100px">
					<a href="../GetPolicySummary.do?TabPanelName=Policy_Panel&answer(policyNumber)=<%=""+policyNumber%>&answer(Object::Quote::PaymentPlan)=<%=""+paymentPlan %>"> Policy</a>			
				</td>
...
<!--########################## PAYMENT PLAN	 #########################################-->
				<dmshtml:get_Installment_Types	nameInSession="arrInstallmentTypes"	productVerId="<%=""+ProductVerId %>" dbname="<%= com.dms.web.util.DbUtils.getReadDS(pageContext)%>" />
								<logic:present name="arrInstallmentTypes">
										...
										%>
								<td class="links" align="left" width="150px">
									<a href="../LoadPaymentPlanAction.do?TabPanelName=Quote_Panel&answer(Object::Product::ProductVerId)=<%=ProductVerId+""%>&answer(Object::Quote::QuoteStatus)=<%=paretQuoteStatus.toString()%>&answer(Object::Quote::ParentQuoteId)=<%=parentQuoteId+"" %>&answer(Object::Quote::subQuoteId)=<%=subQuoteId+"" %>&answer(Object::Customer::customerId)=<%=customerId+"" %>&answer(Object::Quote::ProductId)=<%=prodId+"" %>&answer(Object::Quote::groupId)=<%=GroupId+"" %>&answer(Object::Quote::groupVerId)=<%=GroupVerId+"" %>&answer(Object::Quote::productStatus)=<%=productStatus+"" %>&answer(Object::Quote::subQuoteVerId)=<%=subQuoteVerId+"" %>&answer(Object::Quote::parentQuoteVerId)=<%=parentQuoteVerId+"" %>&answer(Object::Quote::PrevSubQuoteVerId)=<%=""+subQuoteVerId%>&answer(Object::Quote::TotalPremium)=<%=TotalParentQuotePremium+""%>&answer(Object::QuoteBilling::InstallmentDetail)=<%=qMap.getString("Object::QuoteBilling::NumberOfInstallments")+"::"+qMap.getString("Object::QuoteBilling::InitialPaymentPercentage")+"::"+qMap.getString("Object::QuoteBilling::InstallmentFee")+"::"+qMap.getString("Object::QuoteBilling::InstallmentTypeDesc")%>" title="Click to go to add Payment"/>Payment plan</a>
								</td>			
								</logic:present>
<!--########################## PAYMENT #########################################-->
...
<!-- 	<dmshtml:dms_isPresent property="<%=new String[]{"Unlocked"}%>" value="<%=paretLockStatus.toString()%>" negated="true">
		<dmshtml:isFeatureAvailableForProductRoleStatus roleId='<%=Long.parseLong(""+objDescId) %>' productId='<%=Long.parseLong(""+prodId) %>' dbName='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>' featureId="PROPOSAL_AVAILBLE" 
			property='<%=new String[]{"In Progress"}%>' value="<%=paretQuoteStatus.toString()%>" negated="true">
			<td class="sectionhead" align="left" width="12%">Plan</td>
			</dmshtml:isFeatureAvailableForProductRoleStatus>
	</dmshtml:dms_isPresent> -->	
...
<!-- 		<td class="sectionhead" align="left">Risk Number</td>
		...
		<td class="sectionhead" align="left" width="13%">Risk List</td> 
...
<dmshtml:dms_isPresent property="<%=new String[]{"Unlocked"}%>" value="<%=paretLockStatus.toString()%>" negated="true">			
	...
	<!-- -------------------------------Policy --------------------------------- -->
			...
			<!-- <a href="../GetRiskListCL.do?TabPanelName=Quote_Panel&answer(parentQuoteId)=<%=parentQuoteId.toString()%>&parentQuoteId=<%=parentQuoteId.toString()%>&answer(subQuoteId)=<%=subQuoteId.toString()%>&answer(QuoteId)=<%=subQuoteId.toString()%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyId.toString()%>&subQuotePlanName=<%=subQuoteProductId%>&answer(parentQuoteVerId)=<%=parentQuoteVerId.toString()%>&answer(Object::Plan::CompositeRateFlag)=N&answer(sortKey)=EMP_ID">Risk List</a> -->
			<a href="../GetRiskListCL.do?TabPanelName=Quote_Panel&answer(TabPanelName)=Quote_Panel&answer(IsApplicationDisabled)=<%=""+IsApplicationDisabled%>&customerId=<%=""+customerId%>&answer(Object::UserDetail::ownerId)=<%=""+ownerId %>&answer(RiskType)=COMMERCIAL&SearchObjectId=<%=""+customerId%>&answer(Object::Customer::CustomerId)=<%=""+customerId%>&answer(Object::Risk::ComProperty::CustomerId)=<%=""+customerId%>&answer(Object::Agency::AgencyId)=<%=""+agencyId%>&answer(objectId)=<%=""+prodId%>&answer(objectVerId)=<%=""+ProductVerId%>&answer(objectName)=PRODUCT&answer(applicationType)=RISK&answer(applicationSubType)=&Navigation=&answer(prdGroupId)=<%=""+GroupId%>&answer(prdGroupVerId)=<%=""+GroupVerId%>&answer(productStatus)=<%=""+productStatus%>&answer(teritory)=<%=""%>&answer(riskTerritory)=<%=""+riskTerritory%>&answer(territoryName)=<%=""%>&answer(zone)=<%=""%>&answer(state)=<%=""%>&answer(showOnetTab)=Y&tabNo=0&answer(Object::Employee::PageNum)=0&answer(Object::Customer::State)=<%=""+CustomerState%>&answer(Object::Risk::ComProperty::CustomerId)=<%=""+customerId%>&answer(Object::Risk::ComProperty::ComPropertyId)=-1&answer(SelectedRiskType)=COMMERCIAL<%=" "%>AUTO&answer(parentQuoteId)=<%=parentQuoteId.toString()%>&parentQuoteId=<%=parentQuoteId.toString()%>&answer(subQuoteId)=<%=subQuoteId.toString()%>&answer(QuoteId)=<%=subQuoteId.toString()%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyId.toString()%>&subQuotePlanName=<%=subQuoteProductId%>&answer(FamilyId)=<%=ProductFamilyId.toString()%>&answer(parentQuoteVerId)=<%=parentQuoteVerId.toString()%>&answer(Object::Quote::QuoteStatus)=<%=paretQuoteStatus.toString()%>&answer(subQuoteVerId)=<%=""+subQuoteVerId%>&answer(Object::Quote::LockStatus)=<%=paretLockStatus.toString()%>">Risk List</a>
			</td>
			<!-- ################################# RISK NAME END ############################### -->
			
			<td class="SearchResult">
			
				<a href="../PlanChange.do?TabPanelName=Quote_Panel&answer(IsOptionDisabled)=<%=""+IsOptionDisabled%>&changePlan=yes&answer(Object::Risk::Teritory)=<%=""+riskTerritory%>&answer(parentQuoteId)=<%=parentQuoteId.toString()%>&answer(parentQuoteVerId)=<%=parentQuoteVerId.toString()%>&answer(QuoteId)=<%=subQuoteId.toString()%>&answer(subQuoteId)=<%=subQuoteId.toString()%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyId.toString()%>&answer(ownerId)=<%=ownerId.toString()%>&answer(userId)=<%=userId.toString()%>&answer(Plan::PlanNumber)=<%=""+subQuoteId%>&answer(Object::Quote::ProductId)=<%=subQuoteProductId%>&answer(Option::DefaultValue)=No&answer(ProductId)=<%=""+prodId%>&answer(ProductVerId)=<%=""+ProductVerId%>&answer(prdGroupId)=<%=""+GroupId%>&answer(prdGroupVerId)=<%=""+GroupVerId%>&answer(productStatus)=<%=""+productStatus%>&answer(subQuoteVerId)=<%=""+subQuoteVerId%>&answer(customerId)=<%=""+customerId%>&answer(Object::Quote::PrevSubQuoteVerId)=<%=""+subQuoteVerId%>">Coverages and Limits</a>
...
<!-- ################################# Preview Binder PDF Start ############################### -->
			...
			property='<%=new String[]{"In Progress","Offered","Bind Request","Submitted"}%>' value="<%=paretQuoteStatus.toString()%>" negated="true">
				...
				<td class="SearchResult">			
					<a href="../GetDocumentListCL.do?TabPanelName=Quote_Panel&answer(DocumentType)=ISSUANCE&answer(parentQuoteId)=<%= ""+parentQuoteId%>&answer(subQuoteId)=<%= ""+subQuoteId%>&answer(subQuoteVerId)=<%= ""+subQuoteVerId%>&answer(OwnerId)=<%=ownerId.toString()%>&answer(ProductVerId)=<%=""+ProductVerId%>&answer(Object::Customer::CustomerId)=<%=""+customerId%>&answer(customerId)=<%=""+customerId%>&answer(ProductId)=<%=""+prodId%>&answer(prdGroupId)=<%=""+GroupId%>&answer(prdGroupVerId)=<%=""+GroupVerId%>&answer(productStatus)=<%=""+productStatus%>">Schedule of Forms</a>
				</td>
			...
			<td class="SearchResult">
				...
				<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"QUOTE_WORKSHEET_EDIT"}%>' connector='or' negated="true"> 
						...
						<a	href="../GetUWWorkSheetAction.do?TabPanelName=Quote_Panel&answer(productId)=<%=""+subQuoteProductId %>&answer(productVerId)=<%=""+subQuoteProductVerId %>&answer(prevSubQuoteVerId)=<%=""+subQuoteVerId%>&answer(Object::Quote::QuoteType)=<%=QuoteType%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(QuoteId)=<%=subQuoteId.toString()%>&answer(ownerId)=<%=ownerId.toString()%>&answer(parentQuoteId)=<%=parentQuoteId.toString()%>&answer(parentQuoteVerId)=<%=parentQuoteVerId.toString()%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(subQuoteId)=<%=subQuoteId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyId.toString()%>&answer(customerNumber)=<%=EmpGrpNum.toString()%>&answer(Object::Quote::PaymentPlan)=<%=""+paymentPlan %>&answer(modQuoteNumber)=<%=""+QuoteNumber%>&answer(state)=<%=""+planState%>">Worksheet </a>
