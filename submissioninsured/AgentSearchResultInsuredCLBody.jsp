...
<html:form action="/LinkAgencytoInsuredSubCLAction.go">
					...
					<tr>
						<td  class="links"  colspan="4">
							<logic:present name="prevEntity"  scope="request" >
								<a href="../LinkAgenttoInsuredAction.do?TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Entity::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>"><bean:message key="Common.Previous"/></a>
							</logic:present>
						</td>
						<td align="right" class="links" colspan="3">
							<logic:present name="moreEntity"  scope="request" >
								<a href="../LinkAgenttoInsuredAction.do?TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Entity::PageNum)=<%=(Integer.parseInt(""+pagenum)+1)%>">Next&gt;&gt;</a>
							</logic:present>
						</td>
					</tr>
						...
						<bean:define id='test' name="searchresultnewDisplay"  property="answer(Object::Entity::EntityId)" />
							<tr>
	   							<td class=<%= styleClass %>>
									...
									<%	  
									agentSelTemp = ""+agentSel;
									if(!agentSelTemp.equalsIgnoreCase("")){
