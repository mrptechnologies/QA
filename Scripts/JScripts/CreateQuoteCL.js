...
var attributeHtmlArr=AttributesHtml.split("$html$");
	...
	document.getElementById("CreateQuoteSubmit").disabled = false;
}
...
//validate date function - this is specifically for BTA Binder

function validateDateBinderBTA(submit){
	var frm = document.forms[0];
	var vardate = frm.policyAnniversaryDate.value;
	var expDate = frm.policyExpirationDate.value;
	...
	{
		alert("Please enter the Effective Date");
		frm.policyAnniversaryDate.focus();
		return false;
	}
	if (expDate=="")
	{
		alert("Please enter the Expiry Date");
		frm.policyExpirationDate.focus();
		return false;
	}
	...
	var emonth = parseInt(expDate.substring(0, 2), 10);
	var eday = parseInt(expDate.substring(3, 5), 10);
	var eyear = parseInt(expDate.substring(6, 10), 10);

	if (isNaN(dmonth) || dmonth < 1 || dmonth > 12 || isNaN(dday) || dday < 1 || dday > 31 || isNaN(dyear) || vardate.substring(2, 3)!="/" || vardate.substring(5, 6)!="/")
	{
		alert("Please enter date in 'MM/DD/YYYY' format");
		frm.policyAnniversaryDate.select();
		return false;
	}
	
	if (isNaN(emonth) || emonth < 1 || emonth > 12 || isNaN(eday) || eday < 1 || eday > 31 || isNaN(eyear) || expDate.substring(2, 3)!="/" || expDate.substring(5, 6)!="/")
	{
		alert("Please enter date in 'MM/DD/YYYY' format");
		frm.policyExpirationDate.select();
		return false;
	}	
