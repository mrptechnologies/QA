<%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean" %>
<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html" %>
<%@ taglib uri="/WEB-INF/struts-logic.tld" prefix="logic" %>
<%@ taglib uri="/WEB-INF/dms-html.tld" prefix="dmshtml" %>
		<bean:define id="ownerId" property="answer(Object::UserDetail::ownerId)" name="LoginPage" scope="session"/>
<html:form action="/LinkAgencytoInsuredSubAction1.go">
<div class="formpage01">
	<table WIDTH="100%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
	    <tr>
			<td><div class="formhead"><h2 class="left">AGENT LIST</h2></div></td>
		</tr>


		<tr><td>&nbsp;</td></tr>

		<bean:define id="pagenum" name='pagenum' scope='request'/>
		<bean:define id="totpagenum" name='totpagenum' scope='request'/>
		<%String strcnt="";%>
		<logic:present name ='countSearchResult' scope ='request'>
			<bean:define id="countSearchResult1" name="countSearchResult" scope="request"/>
			<%strcnt=""+countSearchResult1;%>
		</logic:present>


		<tr>
			<td class="sectionhead" >
				Total Number of Agent(s) Found &nbsp;&nbsp;<%=strcnt%>
			</td>
		</tr>
		<tr><td>&nbsp;</td></tr>
		<tr>
			<td class="sectionhead"  align="center">
				Page <%=(Integer.parseInt(""+pagenum)+1)%> of <%=(Integer.parseInt(""+totpagenum))%>
			</td>
		</tr>

		<tr>
			<td valign="top">
				<table WIDTH="100%" ALIGN="center" class="loctable" CELLSPACING="0" CELLPADDING="0">
					<tr>
						<td colspan="4">
							<logic:notPresent name ='searchresultnewDisplay' scope ='request'>
								<span class="Error">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Currently no Agents are listed</span>
								<table WIDTH="100%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
								<tr><td>&nbsp;</td></tr>

								<tr>
									<td>No.</td>
	 								<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"DATABASE-ID-VIEW"}%>' connector='or' negated="true">	
										<td>Agent/Broker Contact Id</td>
										<td>Agent Number</td>
									</dmshtml:dms_static_with_connector>
									<td>Agent First Name</td>
									<td>Agent Last Name</td>
									<td>Select</td>
								</tr>
								
								<tr><td></td></tr>
								
								<tr>
									<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"DATABASE-ID-VIEW"}%>' connector='or' negated="true">	
										<td class="altdisply">
											<%="1"%>
										</td> 
										<td class="altdisply">-1</td>
									</dmshtml:dms_static_with_connector>
								
									<td class="altdisply">-1</td>
										
									<td class="altdisply" >None</td>
										
									<td class="altdisply" >None</td>
								
									<td class="altdisply">												
										<input type="radio" name="answer(AgentId)" value="-1" checked="checked" >
									</td>
								</tr></table>		
							</logic:notPresent>

						</td>
					</tr>

					<tr><td>&nbsp;</td></tr>
 

				<logic:present name ='searchresultnewDisplay' scope ='request'>


					<bean:define id="pagenum" name='pagenum' scope='request'/>
					<bean:define id="totpagenum" name='totpagenum' scope='request'/>

	<!--  -->
					<tr>
						<td colspan="5" align="right" >
							<html:submit value="Select" styleClass="covbutton"/>
						</td>
					</tr>
					
					<tr>
						<td colspan="4" align="right" >&nbsp;</td>
					</tr>
	<!--  -->

					<tr>
						<td  class="links"  colspan="4">
							<logic:present name="prevEntity"  scope="request" >
								<a href="../LinkAgenttoInsuredSubAction1.do?TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Entity::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>"><bean:message key="Common.Previous"/></a>
							</logic:present>
						</td>
						<td align="right" class="links" colspan="3">
							<logic:present name="moreEntity"  scope="request" >
								<a href="../LinkAgenttoInsuredSubAction1.do?TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Entity::PageNum)=<%=(Integer.parseInt(""+pagenum)+1)%>">Next&gt;&gt;</a>
							</logic:present>
						</td>
					</tr>


					<tr>
						<td>No.</td>
						<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"DATABASE-ID-VIEW"}%>' connector='or' negated="true">	
							<td>Agent/Broker Contact Id</td>
							<td>Agent Number </td>
						</dmshtml:dms_static_with_connector>
						<td>Agent First Name</td>
						<td>Agent Last Name</td>
						<td>Select</td>
					</tr>

					<% int i=0;%>
					<% int count=0; %>
					
					<%
					String pageNum = (String)request.getAttribute("pagenum");
					try{
					    i=Integer.parseInt(pageNum);
							i = i * 20;
					  }catch(NumberFormatException nfe)
					  {
						  i=0;
					  }
					
					%>
	
					<% boolean checked=false; %>


					<logic:iterate id="searchresultnewDisplay" name="searchresultnewDisplay" scope="request" >
						<% count++; %>
						<%int pageCount =Integer.parseInt(""+pagenum); %>
						<% 		
							String styleClass="";
							if(count%2==0){
								styleClass = "altdisply";
							}
							else{
								styleClass = "SearchResult";	
							}
						%>
						<bean:define id='test' name="searchresultnewDisplay"  property="answer(Object::Entity::EntityId)" />
							<tr>
	   							<td>
	   								<%=++i%>
									<bean:define id="agentSel" name="AddCustomer" property="answer(AgentId)" scope="session"/>

									<%	  
									if(Long.parseLong(agentSel.toString()) > 0)
									
									{
									    checked=false;
									}
									else{
									    if(i==1 || i == ((pageCount*20)+1))
									        
									    {
									       checked=true ;
									    }
									}
									%>
								</td>
								
											
										<%	  
										if(  Long.parseLong(agentSel.toString()) ==Long.parseLong(test.toString()) )
										
										{
										    checked=false;
										}
										else{
										    if(i==1 || i == ((pageCount*20)+1))
										        
										    {
										       checked=true ;
										    }
										}
										%>
								<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"DATABASE-ID-VIEW"}%>' connector='or' negated="true">	
									<td>
										<%=test.toString()%>
									</td> 
									<td>
									<bean:write name="searchresultnewDisplay" property="answer(Object::Entity::EntityNum)" />&nbsp;
								</td>
								</dmshtml:dms_static_with_connector>
								
								
								<td>
									<bean:write name="searchresultnewDisplay" property="answer(Object::Entity::FirstName)" />&nbsp;
								</td>
		
								<td>
									<bean:write name="searchresultnewDisplay" property="answer(Object::Entity::LastName)" />&nbsp;
								</td>

		
								<td>
									<%
							       			if(checked){ 
							                %> 
									<input type="radio" name="answer(AgentId)" value="<%=test.toString()%>" checked="checked" >
							                <%
							                        }else{
							                %> 
									<html:radio property="answer(AgentId)" value="<%=test.toString()%>" >
									</html:radio>
							                <%
							                        }
							                %>
								
								</td>
							</tr>		
					</logic:iterate>
					<tr>
						<td class="altdisply">
							<%=(i+1)%>
						</td>
						<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"DATABASE-ID-VIEW"}%>' connector='or' negated="true">	
							<td class="altdisply"><%="-1"%></td> 
								<td class="altdisply"><%="-1"%></td> 
						</dmshtml:dms_static_with_connector>
		
						<td class="altdisply" >None</td>
		
						<td class="altdisply" >None</td>

						<td class="altdisply" width="10">
							<dmshtml:dms_radio property="answer(AgentId)" value="-1" >
							</dmshtml:dms_radio> 
						</td>
						<% i=0;%>
					</tr>


					<tr>
						<td  class="links"  colspan="4">
							<logic:present name="prevEntity"  scope="request" >
								<a href="../LinkAgenttoInsuredSubAction1.do?TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Entity::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>"><bean:message key="Common.Previous"/></a>
							</logic:present>
						</td>
						<td align="right" class="links" colspan="3">
							<logic:present name="moreEntity"  scope="request" >
								<a href="../LinkAgenttoInsuredSubAction1.do?TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Entity::PageNum)=<%=(Integer.parseInt(""+pagenum)+1)%>">Next&gt;&gt;</a>
							</logic:present>
						</td>
					</tr>
					
				</logic:present>

<!--  -->
					<tr>
						<td colspan="4" align="right" >&nbsp;</td>
					</tr>
					
					<tr>
						<td colspan="5" align="right" >
							<html:submit value="Select" styleClass="covbutton"/>
						</td>
					</tr>
<!--  -->
				</table>
			</td>
		</tr>
	</table>
<html:hidden property="TabPanelName" value="<%=""+request.getParameter("TabPanelName") %>" />
</div>
</html:form>

