...
<head>
<title><%=session.getAttribute("PageTitle")%> - Link ProductOptions To State</title>

</head>
<html:form action="/LinkProductOptionToState">
...
<tr><td>&nbsp;</td></tr>
<tr><td class="MH">
LINK PRODUCT OPTIONS AND STATE
</td></tr>
<tr><td class="TextMatter">
Use this page to link product Options to state
<br>
</td></tr>
...
<td class="FormLabels" align="left">Select Option(s) to be linked to the state</td></tr>
<tr><td>
...
<tr>
		<td width="30%"><dmshtml:get_available_prod_state_options nameInSession="arrProdOptions" ownerId="<%=Long.parseLong(""+ownerId)%>" productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>"
state="<%=request.getParameter("answer(Object::Product::State)").toString()%>"
dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::ProductFeature::ProductOptionId)" styleId="possible" multiple="true" size="7" >
...
<td width="30%"><dmshtml:get_prod_state_options nameInSession="arrProdOptions" ownerId="<%=Long.parseLong(""+ownerId)%>" productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>"
state="<%=request.getParameter("answer(Object::Product::State)").toString()%>"
dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::ProductFeature::ProductOptionId)" styleId="chosen" multiple="true" size="7" >
			<logic:present name="arrProdOptions" scope="request">
           <logic:iterate id="prodOptions" name="arrProdOptions" scope="request" >
		    <bean:define id='optionName' name="prodOptions"  property="optionName" />
		    <bean:define id='optionId' name="prodOptions"  property="optionId" />
...
<html:hidden property="answer(objectType)" value="stateOption" />
