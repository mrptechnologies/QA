...
<html:form action="/LinkAgenttoInsuredAction.do" >
<html:hidden property="Object::AGENCY::BUSINESSNAME" value="" styleId="StyleBusinessName" />
<bean:define id="parentQuoteID" name="AddCustomer" property="answer(parentQuoteId)" />
	...
	<table WIDTH="98%" ALIGN="center" BORDER="0" CELLSPACING="2" CELLPADDING="0">
		...
		<%if(parentQuoteID != null && parentQuoteID != "") { System.out.println("parentQuoteID"+parentQuoteID);%>
		<tr>
		<td>
		...
		<tr><td>
			...
			<table WIDTH="100%" ALIGN="Left" cellspacing="2" cellpadding="2">
				...
				<tr>
					<td align="Left" class="links" colspan="5" >
						<logic:present name="firstAgency"  scope="request" >
							<a href="../AgencySearchInsuredAction.do?answer(IsPopupEnabledAgencyFind)=<%=""+request.getParameter("answer(IsPopupEnabledAgencyFind)")%>&formNameAgencyFind=<%=""+request.getParameter("formNameAgencyFind")%>&TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Agency::PageNum)=0">&lt;&lt;First</a>
						</logic:present>&nbsp;&nbsp;
						<logic:present name="prevAgency"  scope="request" >
							<a href="../AgencySearchInsuredAction.do?answer(IsPopupEnabledAgencyFind)=<%=""+request.getParameter("answer(IsPopupEnabledAgencyFind)")%>&formNameAgencyFind=<%=""+request.getParameter("formNameAgencyFind")%>&TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Agency::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>"><bean:message key="Common.Previous"/></a>
						</logic:present>
					</td>
					
					<td align="right" class="links" colspan="5">
						<logic:present name="moreAgency"  scope="request" >
							<a href="../AgencySearchInsuredAction.do?answer(IsPopupEnabledAgencyFind)=<%=""+request.getParameter("answer(IsPopupEnabledAgencyFind)")%>&formNameAgencyFind=<%=""+request.getParameter("formNameAgencyFind")%>&TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Agency::PageNum)=<%=(Integer.parseInt(""+pagenum)+1)%>">Next&gt;&gt;</a>
						</logic:present>&nbsp;&nbsp;
						<logic:present name="lastAgency"  scope="request" >
							<a href="../AgencySearchInsuredAction.do?answer(IsPopupEnabledAgencyFind)=<%=""+request.getParameter("answer(IsPopupEnabledAgencyFind)")%>&formNameAgencyFind=<%=""+request.getParameter("formNameAgencyFind")%>&TabPanelName=<%=""+request.getParameter("TabPanelName") %>&answer(ownerId)=<%=""+ownerId %>&LimitSearch=yes&answer(Object::Agency::PageNum)=<%=(Integer.parseInt(""+totpagenum)-1)%>">Last&gt;&gt;</a>
						</logic:present>   
					</td>
				</tr>
				...
				<tr>
					...
					<bean:define id="businessNameSel" property="Object::AGENCY::BUSINESSNAME" value="<%=qmap.getString("Object::AGENCY::BUSINESSNAME")%>" />
					</td>
					...
					<td class=<%= styleClass %> align="center" >
						...
						<INPUT TYPE="radio" name="answer(Agency::AgencyId)" value="<%=qmap.getString("Object::AGENCY::AGENCYID")%>"  checked onclick="setBusinessName('<%=""+businessNameSel.replaceAll("'","\\\\'")%>')"/>
					       <script type="text/javascript">	
					       <% String businessNameEnc = businessNameSel.replaceAll("'","\\\\'");%>
								setBusinessName('<%=""+businessNameEnc%>');
							</script>	
					        <%checked=false;
					                }else {
					        %> 
						<INPUT TYPE="radio" name="answer(Agency::AgencyId)" value="<%=qmap.getString("Object::AGENCY::AGENCYID")%>" onclick="setBusinessName('<%=""+businessNameSel.replaceAll("'","\\\\'")%>')"/>
