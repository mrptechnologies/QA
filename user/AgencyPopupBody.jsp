...
function submitToParent(frm)
 {
	 if(tempAgencyName==null){
		 ...
		 window.opener.document.<%=frmName%>.AgencyName.value = tempAgencyName;
...
<html:form action="/AgencyForAgentSearchPage1.do"  styleId="searchAgency" onsubmit="checkForInvalidSessionCloseAndForward('N');">
...
<!-- ################################################################################################ -->
	...
	<html:form action="/AgencyForAgentSearchPage1.do" styleId="getAgencyDetail" onsubmit="if(submitToParent(this)) {checkForInvalidSessionCloseAndForward('Y');} else {return false;}">
		...
		%>
				...
				<bean:parameter id="strState" name="answer(Address::Equals::State)"/><tr>
					...
					<a href="../AgencyForAgentSearchPage1.do?&LimitSearch=yes&answer(Address::Equals::State)=<%=strState%>&answer(Object::Agency::TotalRec)=<%=totrec%>&answer(Agency::Like::BusinessName)=<%=strAgentName%>&FormName=<%=frmName%>&answer(Object::Agency::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>&answer(Agency::orderBy)=<%=encodedOrderBy%>" onclick="checkForInvalidSessionCloseAndForward('N');">&lt;&lt;Previous</a></td></tr>
				</logic:present>
		...
		<table width="100%" align="center" border="0" CELLSPACING="2" CELLPADDING="0">
			...
			for(int i=0; i<agencyArrList.size();i++)
			{
				...
				%>
					<tr>
						<!-- <td class="SearchResult" name="AddAgent" scope="request"  align="center">
							 <%=(i+1)%>
						</td> -->
						...
						<td class="SearchResult"><%=qmap_Temp.getEncodedString("Object::AGENCY::BUSINESSNAME")%> 
						<input type="hidden" id="SelectedAgencyNum" value='<%=qmap_Temp.getString("Object::AGENCY::BUSINESSNAME")%>'/>
						</td>
						<td class="SearchResult"  align="center">			
						<INPUT TYPE="radio" value="<%=(i)%>" name="AddAgent" onclick="return selectValue(this)"/>
						</td>
					</tr>
			<%}
			%>
