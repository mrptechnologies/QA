...
function addRisk(){
	...
	var tankDesc = escape(document.getElementById("LocationDescription").value);
	...
	'&answer(Object::Submission::InsuredId)='+submissionInsuredId+'&answer(Object::Submission::Type)='+submissionType+
...
var count1=0;
 function appendLocationToParent(locId,newLocName){
		...
		tsrLocationName = tsrLocationName.replace(/&lt;/g,"<");
		tsrLocationName = tsrLocationName.replace(/&gt;/g,">");
		var parent = document.getElementById("divide1");
		//alert(tsrLocationName)
		...
		onException:function(){},
 		onComplete:function(transport){
			if(document.getElementById("LocationAndTanksList") != null && document.getElementById("LocationAndTanksList") != undefined) {
				forwardToLoginPageIfInvalidSession(document.getElementById("LocationAndTanksList").innerHTML);
			}
 		document.getElementById("Loc3TableData").style.display = "none";
		document.getElementById("AddLocationDiv").style.display = "none";
		document.getElementById("AddLocationDiv").innerHTML = " ";
...
//Div page for Add location
	...
	autologout();
	new Ajax.Updater({success: 'AddLocationDiv'},'../submission/AddLocationUI3.jsp',{method:'post',onComplete:function(transport){
		if(document.getElementById("AddLocationDiv") != null && document.getElementById("AddLocationDiv") != undefined) {
			forwardToLoginPageIfInvalidSession(document.getElementById("AddLocationDiv").innerHTML);
	} ShowAddLocationDiv1(id);},onSuccess: function(transport){}});
...
function appendtable1()
{
	...
	index++;
		...
		parent.appendChild(anc);
	//	td1.setAttribute("id","NewLocationName");
		anc.appendChild(document.createTextNode("New Location"));
		updateDivForAddLocation();
		activateTabs(id);
		addLocationDivActive = true;
	}else{
...
function updateDivForAddLocation(){
	...
	var pars1 = 'answer(objectId)='+objectId+'&answer(objectVerId)='+objectVerId+'&answer(applicationType)=LOCATION&answer(objectType)="location"&answer(Object::Location::LocationId)=-1&answer(Object::Application::ApplicationType)=SUBMISSION_APPLICATION&OwnerId='+ownerId+
	...
	'&answer(State)=&answer(zone)=&answer(TabPanelName)='+panelName+
	'&answer(Object::Submission::InsuredId)='+custId+'&answer(Object::Submission::SubmissionStatus)='+submissionStatus+'&answer(Object::Submission::Type)='+submissionType+
	...
	autologout();
	new Ajax.Request(url, {method: 'post',onComplete:function(transport){forwardToLoginPageIfInvalidSession(transport.responseText);},onSuccess: function(transport){showTab(transport);}});
...
function getSpecificQuestions(tankType){
	...
	'&answer(PrimaryKey_Object::Customer::CustomerId)='+custId+'&answer(PrimaryKey_Object::Submission::SubmissionId)='+submissionId+
	'&answer(PrimaryKey_Object::Location::LocationId)='+locationId+'&answer(Object::Location::LocationId)='+locationId+'&answer(Object::Application::ApplicationType)='+Applicationtype+
	'&answer(Object::Customer::CustomerId)='+custId+'&answer(Object::Submission::InsuredId)='+custId+'&answer(actionName)='+actionName+'&answer(SubmitType)='+submitType+
	'&answer(Object::Submission::SubmissionId)='+submissionId+'&answer(Object::Customer::SubmissionId)='+submissionId+'&answer(riskType)='+objectName+'&answer(objectType)='+objectType;
...
function geteditRiskPopUp(tankId,riskType){
	...
	'&answer(objectName)='+objectName+'&answer(Object::Application::OwnerId)='+ownerId+'&answer(ButtonName)='+ButtonName+'&answer(Object::Submission::SubmissionStatus)='+submissionStatus+'&answer(Object::Submission::Type)='+submissionType+
	...
	var url = '../addTankRiskApp.do?'+pars1;		
	autologout();
	new Ajax.Request(url, {method: 'post',onComplete:function(transport){forwardToLoginPageIfInvalidSession(transport.responseText);},onSuccess: function(transport){showRiskPopUp(transport,locationId,heading);}});
