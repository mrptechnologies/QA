...
<html:form action="/LinkFeatureToProduct">
...
%>
	...
	<table WIDTH="95%" ALIGN="center" BORDER="0" CELLSPACING="2" CELLPADDING="0">
		...
		<tr>
			<td class="MH">LINK FEATURES AND PRODUCT</td>
		</tr>
		<tr>
			<td class="TextMatter">Use this page to link features to product
			<br>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<table WIDTH="550px" ALIGN="center" cellspacing=0 cellpadding="2"
				...
				<tr>
					<td class="FormLabels" align="left">Feature</td>
				...
				<bean:define id="ownerId"
					...
					scope="session" />
				<tr>
					<td width="15%"><dmshtml:get_available_features
						nameInSession="arrFeatures"
					...
					<dmshtml:dms_select property="answer(Object::Product::Feature)"
						...
						<logic:present name="arrFeatures" scope="request">
							<logic:iterate id="features" name="arrFeatures" scope="request">
								<bean:define id='featureId' name="features"
									property="productFeatureId" />
								<dmshtml:dms_option value='<%=featureId.toString()%>'>
									<bean:write name="features" property="featureDesc" />
								</dmshtml:dms_option>
							</logic:iterate>
						</logic:present>
					...
					<td width="30%"><dmshtml:get_product_features
						nameInSession="arrFeatures"
						ownerId="<%=Long.parseLong(""+ownerId)%>"
						productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>"
						dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>" />
	...
	<html:hidden property="answer(objectType)" value="Feature" />
