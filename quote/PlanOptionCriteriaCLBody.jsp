...
<dmshtml:GetRequestedCoverages nameInSession="RequestedCoverages" submissionId="<%=Long.parseLong(""+submissionId)%>" submissionVerId="<%=Long.parseLong(""+submissionVerId)%>" ownerId="<%=Long.parseLong(""+ownerId)%>" DBName='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>'/>
					...
					<%String isRetroActivedateVaries=""; %>
					<%String isDeductibleVaries=""; %>										
					<%double deductible = 0; %>
	...
	<logic:present name="RequestedCoverages">
						...
						<% if(submissionCoveragesDetail.getEffDate() != null ){
						   ...
						   if(""+submissionCoveragesDetail.getIncidentLimit() !=null){
							...
							if(""+submissionCoveragesDetail.getIsRetroActiveDateVaries() !=null){
								isRetroActivedateVaries = submissionCoveragesDetail.getIsRetroActiveDateVaries();
							}
							if(""+submissionCoveragesDetail.getIsDeductibleVaries() !=null){
								isDeductibleVaries = submissionCoveragesDetail.getIsDeductibleVaries();
							}							
							double dbAbount=0.0;
							String usCurrency = "";
...
<h3>Plan Name&nbsp;&nbsp;&nbsp;&nbsp;<%=ProductGroupName%></h3> 
...
<html:hidden property ="answer(pagename)" value="Coverages and limits" />
			...
			<bean:define id="uwNameCode" name="AddParentQuote" property="answer(Object::Quote::QuoteUWNameCode)" scope="request"/>
			
			<html:hidden property="answer(Plan::AddPlan)"  name="AddParentQuote" value="<%=""+AddPlan%>" />
...
<%}else{ %>
<html:text property="answer(Object::Plan::StateCode)" value="<%=stateFormRequest.toString()%>" />
<%} %>
...
<logic:notEqual name="IsOptionDisabled" value="Y">
	...
	<div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'>
		...
		<div class='navValue'><INPUT TYPE='submit'  value='Rate' class="covbutton left" id="RateTop" onclick="return goSubmit(document.AddParentQuote.RateTop,'Rate')" name='answer(form::ForwordControl1)'></div>
		</dmshtml:isFeatureAvailableForProductRoleStatus>
	</div>
	<div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'><div class='navValue'><INPUT TYPE='submit' value='Save' class="covbutton left" id="SaveTop" onclick="return goSubmit(document.AddParentQuote.SaveTop,'Save')" name='answer(form::ForwordControl1)' style='color: FF0000'></div></div>
	</div>
...
<html:hidden property="answer(Object::Plan::TermInMonths)" />
	...
	<div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'>
		<dmshtml:isFeatureAvailableForProductRoleStatus roleId='<%=Long.parseLong(""+objDescId) %>' productId='<%=Long.parseLong(""+prodId) %>' dbName='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>' featureId="RATE_AVAILABLE" property='<%=new String[]{""}%>' value="" negated="true">
		<div class='navValue'><INPUT TYPE='submit' value='Rate' class="covbutton left" id="RateBottom" onclick="return goSubmit(document.AddParentQuote.RateBottom,'Rate')" name='answer(form::ForwordControl1)'></div>
		</dmshtml:isFeatureAvailableForProductRoleStatus>
	</div>
	<div style='margin-right: 5pt; display: block; float:left; margin:0 5px 0 0 ;padding-top:0'><div class='navValue'><INPUT TYPE='submit' value='Save' class="covbutton left" id="SaveBottom" onclick="return goSubmit(document.AddParentQuote.SaveBottom,'Save')" name='answer(form::ForwordControl1)' style='color: FF0000'></div></div>
	</div>
