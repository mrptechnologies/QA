...
function EmailPopup() {
	//alert("1")
       ...
       // alert(custName)
        var custEmail = document.getElementById("custEmail1").value;
	...
	var mylink = '../ComposeMail1.do?answer(Object::Notification::Email::ObjectId)='+custId+'&answer(Object::Customer::CustomerName)='+custName+'&answer(Object::Notification::UserDetail::ToEmail)='+custEmail+'&answer(Object::Notification::UserDetail::Email)='+email+'&answer(Object::Notification::UserDetail::RoleName)='+roleName+'&answer(Object::Notification::Email::ObjectVerId)=0&answer(Object::Notification::Email::ObjectType)=EMPLOYER&answer(Object::Notification::Email::NotificationType)=EMAIL&answer(Object::UserDetail::ownerId)='+ownerId+'&answer(Object::Notification::Email::EmailPurpose)=COSE_INIVITATION_EMPLOYER'
							
	var windowname = 'Email';
	//alert(mylink)
	if (! window.focus) {
		return true;
	}
...
<html:hidden styleId="fName" property="answer(Object::Customer::CustomerName)" value="<%=""+CustomerName%>"  />
<bean:define id="email" property="answer(Object::UserDetail::email)" name="LoginPage" scope="session" />
<html:hidden styleId="email1" property="answer(Object::UserDetail::email)" value="<%=""+email%>"  />
<html:hidden styleId="ownerId1" property="answer(Object::UserDetail::ownerId)" value="<%=""+ownerId%>"  />
<bean:define id="userId" property="answer(Object::UserDetail::userName)" name="LoginPage" scope="session" />
...
%>
<dmshtml:get_latest_email_sent nameInSession="lastEmailSent" objectId='<%=Long.parseLong(""+customerId)%>' ownerId='<%=Long.parseLong(""+ownerId)%>' dbname='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>'/>


<% String sentTime = ""+request.getAttribute("lastEmailSent");
String flag = "";
...
<html:hidden styleId="emailSentStatus" property="answer(Object::UserDetail::emailStatus)" value="<%=""+flag%>"  />
<html:hidden styleId="custId" property="answer(Object::Customer::CustomerId)" value="<%=""+request.getParameter("answer(Object::Customer::CustomerId)")%>"  />


<html:hidden styleId="custEmail1" property="answer(Object::Customer::EMailAddress)" value="<%=""+custEmail%>"  />
...
function getEmailStatus(){
	var emailStatus = document.getElementById("emailSentStatus").value;
	//alert(role);
	return emailStatus;
}
