...
<bean:parameter id="QuoteName" name="answer(Object::Quote::QuoteName)"/>
		...
		<%
			...
			if(request.getParameter("answer(suQuoteVerId)") != null) {
				subQuoteVerIdParam = "" + request.getParameter("answer(suQuoteVerId)");
			}
										...
										%>
											...
											<logic:present name="paramValue">
										   		<logic:equal value="CLBinder" name="paramValue">
													<a id="hiddenPreviewEndorsement" href='../CLPreviewBinderPdf.go?TabPanelName=<%=""+request.getParameter("TabPanelName")%>&answer(DocumentType)=BINDER&answer(QuoteId)=<%= ""+strParentQuoteId%>&answer(subQuoteId)=<%= ""+SubQuoteId%>&answer(OwnerId)=<%=ownerId.toString()%>&answer(ProductVerId)=<%=""+ProductVerId%>&answer(Object::Customer::CustomerId)=<%=""+customerId%>&answer(customerId)=<%=""+customerId%> &answer(isPreview)=<%=true%>' style="{visibility: hidden}"></a>
										   		</logic:equal>
									   		</logic:present>	
									   		<logic:present name="paramValue">
										   		<logic:equal value="PCBinder" name="paramValue">	   		
													<a id="hiddenPreviewEndorsement" href='../PreviewBinderPdf.go?TabPanelName=<%=""+request.getParameter("TabPanelName")%>&answer(DocumentType)=BINDER&answer(QuoteId)=<%= ""+strParentQuoteId%>&answer(subQuoteId)=<%= ""+SubQuoteId%>&answer(OwnerId)=<%=ownerId.toString()%>&answer(ProductVerId)=<%=""+ProductVerId%>&answer(Object::Customer::CustomerId)=<%=""+customerId%>&answer(customerId)=<%=""+customerId%> &answer(isPreview)=<%=true%>' style="{visibility: hidden}"></a>
												</logic:equal>
											</logic:present>
											
											<INPUT TYPE="Button" value="Preview Endorsement" class="sbttn1" onclick="window.open(document.getElementById('hiddenPreviewEndorsement'))"/>
											...
											<td align="center" class="links" >
											<dmshtml:isFeatureAvailableForProductRoleStatus roleId='<%=Long.parseLong(""+objDescId) %>' productId='<%=Long.parseLong(""+ProductId) %>' dbName='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>' featureId="BIND_AVAILABLE_NO_DOCUMENT" 
													property='<%=new String[]{"In Progress","Offered","Bind Request"}%>' value="<%=strParetQuoteStatus.toString()%>" negated="true">
													<dmshtml:dms_isPresent property='<%=new String[]{"ENDORSEMENT"}%>' value="<%=QuoteType.toString()%>" negated="true">
														<dmshtml:dms_button value="Cancel" property="answer(back)" styleId="QuoteProposalCancel" styleClass="sbttn" onclick="javascript:history.go(-1);"/>
