...
<bean:define id="ownerIdfromSession" property="answer(Object::UserDetail::ownerId)" name="LoginPage" scope="session" />
...
if(ownerId==0){
	if(!(""+ownerIdfromSession).equals("")){
		ownerId = Long.parseLong(""+ownerIdfromSession);
	}
}
%>
<SCRIPT type="text/javascript" SRC="../Scripts/JScripts/ZipCodeEL.js"></SCRIPT>
...
function submitToParent(frm)
 {
	 ...
	 var formname = frm.parentFormName.value;
      ...
      window.opener.document.<%=frmName%>.Zip.value = tempZipCode; 
     	window.opener.document.<%=frmName%>.Country.value =tempCountry
...
function submitToChild()
 {	
     ...
     frm.ThisCountry.value = window.opener.document.<%=frmName%>.Country.value;   
...
<html:form action="/SearchZipCodeEIL.do"  styleId="searchZipCode" onsubmit="checkForInvalidSessionCloseAndForward('N');return validateRequest(this)">
<html:hidden property="answer(dbname)" value='<%=com.dms.web.util.DbUtils.getWriteDS(pageContext)%>'/>
	<table WIDTH="95%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<tr>
			...
			<td>
			<html:select name="SearchZipCode"  property="answer(Object::ZipCode::Like::Country)"  styleId="ThisCountry">
					<html:option value ="">Select</html:option>
		...
		<tr><td><input name="FormName" type="hidden" value="<%=frmName%>" /></td></tr>
			...
			<html:form action="/SearchZipCodeEIL.do" styleId="getZipCodeDetail" onsubmit="if(submitToParent(this)) {checkForInvalidSessionCloseAndForward('Y');} else {return false;}">
	...
	<!-- Previous and next links - top -->
		...
		<tr><td>&nbsp;</td></tr>
			...
			<logic:present name="prevZip"  scope="request" >
				<td  class="links">
					<a href="../SearchZipCodeEIL.do?&FormName=<%=frmName%>&answer(Object::ZipCode::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>&answer(ownerId)=<%=""+ownerId%>" onclick="checkForInvalidSessionCloseAndForward('N');"><< Previous</a>&nbsp;
				</td>
			</logic:present>

			<logic:present name="moreZip"  scope="request" >
				<td align="right" class="links" colspan="6">
					<a href="../SearchZipCodeEIL.do?&FormName=<%=frmName%>&answer(Object::ZipCode::PageNum)=<%=(Integer.parseInt(""+pagenum)+1)%>&answer(ownerId)=<%=""+ownerId%>" onclick="checkForInvalidSessionCloseAndForward('N');">Next >></a>
			...
			<tr >	
				...
				<td class="sectionhead" width="10%">Country</td>
	...
	for(int i=0; i<arrList.size();i++)
	{
		...
		<tr>
			...
			<td class='<%=""+strStyleClass%>'><%=qmap_Temp.getString("Object::ZipCode::Country")%>
			<input type="hidden" value='<%=qmap_Temp.getString("Object::ZipCode::Country")%>'/>
			</td>
