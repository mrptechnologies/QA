...
function submitToParent(frm)
 {
	 ...
	 var formname = frm.parentFormName.value;
	  ...
	  window.opener.document.<%=frmName%>.StateForList.value = tempStateCode;
...
function submitToChild()
 {
     ...
     frm.ThisState.value = window.opener.document.<%=frmName%>.StateForList.value;   
     
	 if(window.opener.document.<%=frmName%>.StateForList.value==""){
...
<html:form action="/SearchZipCodeList.do"  styleId="searchZipCode" onsubmit="checkForInvalidSessionCloseAndForward('N');return validateRequest(this)">
<html:hidden property="answer(dbname)" value='<%=com.dms.web.util.DbUtils.getWriteDS(pageContext)%>'/>
	<table WIDTH="95%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<tr><td><input name="FormName" type="hidden" value="<%=frmName%>" /></td></tr>
			...
			<html:form action="/SearchZipCodeList.do" styleId="getZipCodeDetail" onsubmit="if(submitToParent(this)) {checkForInvalidSessionCloseAndForward('Y');} else {return false;}">
	...
	<!-- Previous and next links - top -->
		...
		<tr><td>&nbsp;</td></tr>
			...
			<logic:present name="prevZip"  scope="request" >
				<td  class="links">
					<a href="../SearchZipCodeList.do?&FormName=<%=frmName%>&answer(Object::ZipCode::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>&answer(ownerId)=<%=""+ownerId%>" onclick="checkForInvalidSessionCloseAndForward('N');"><< Previous</a>&nbsp;
				</td>
			</logic:present>

			<logic:present name="moreZip"  scope="request" >
				<td align="right" class="links" colspan="6">
					<a href="../SearchZipCodeList.do?&FormName=<%=frmName%>&answer(Object::ZipCode::PageNum)=<%=(Integer.parseInt(""+pagenum)+1)%>&answer(ownerId)=<%=""+ownerId%>" onclick="checkForInvalidSessionCloseAndForward('N');">Next >></a>
