...
*/
	...
	if(validateFactors(frm) && updateCommissionFactorCalculate(frm) && validateBranchCode(frm) && onOutOfFocus('DeductibleFactor')
		&& onOutOfFocus('OccuranceAggregateLimitFactor') && onOutOfFocus('CoverageDOnSiteBIPDFactor') 
  		...
  		&& onOutOfFocus('TRIACoverageFactor') && onOutOfFocus('NonTRIACoverageFactor') && onOutOfFocus('AdjustedPremium')
  		&& onOutOfFocus('GASurchargeFactor')&& onOutOfFocus('GASurchargeFactorZeroValidation')) {
		returnValue =  true;
	}
	if(frm.branchCode != null && frm.branchCode != undefined && frm.branchCode.value != 'PR'){
		if(frm.GASurchargeFactor != null && frm.GASurchargeFactor != undefined){
			frm.GASurchargeFactor.value = '0.0';
		}		
	}else if(frm.BrnchCode != null && frm.BrnchCode != undefined && frm.BrnchCode.value != 'PR'){
		if(frm.GASurchargeFactor != null && frm.GASurchargeFactor != undefined){
			frm.GASurchargeFactor.value = '0.0';
		}		
	}
...
function onOutOfFocus(fieldName) {
	...
	if(fieldName=="AdjustedPremium" && field!=null) {
 	...
 	if(fieldName=="OccuranceAggregateLimitFactor" && field!=null) {
	   ...
	   if(document.getElementById('branchCode')!= undefined && document.getElementById('branchCode')!=null ){
		   var branchCode =  document.getElementById('branchCode').value;
	   }else if(document.getElementById('BrnchCode')!=undefined && document.getElementById('BrnchCode')!=null){
		   var branchCode =  document.getElementById('BrnchCode').value;
	   }
	   if(branchCode == 'PR'){
		   if(fieldName=="GASurchargeFactor" && field!=null && field!= undefined) {
	  			if(field.value=="") {
	   				alert("Guaranty Association Surcharge factor cannot be blank.");
	   				field.select();
	   				return false;
	  			}
	  			if (field.value.length<1 || !field.value.match(/[^\s]/)) {  
	   				alert("Guaranty Association Surcharge factor cannot be empty space.");
	   				field.select();
	   				return false;   
	  			}
	  			if(field.value!="" && isNaN(field.value)) {
	   				alert("Guaranty Association Surcharge factor should be numeric.");
	   				field.focus();
	   				return false;
	  			}
	  			if(field.value!="" && field.value < 0 ) {
	   				alert("Guaranty Association Surcharge factor cannot be negative.");
	   				field.focus();
	   				return false;
	  			}
	  		}
			var field = document.getElementById("GASurchargeFactor");
			if(fieldName=="GASurchargeFactorZeroValidation" && field!=null && field!= undefined) {	  			
	  			if(field.value!="" && field.value == 0 ) {
	   				alert("Guaranty Association Surcharge factor cannot be zero.");
	   				field.focus();
	   				return false;
	  			}
	  			if((field.value!="" && field.value<0.1) || (field.value!="" && field.value>1.0) ) {
	   				alert("Guaranty Association Surcharge factor should be 0.1% to 1.0%");
	   				field.focus();
	   				return false;
	  			}
	  		}
