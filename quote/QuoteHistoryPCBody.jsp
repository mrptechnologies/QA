		...
		<table WIDTH="100%" ALIGN="center" BORDER="0" CELLSPACING="2" CELLPADDING="0">
			...
			<tr>
				...
				<td class="links" align="left" colspan="2" width="20%">
					<a href="/AddResProperty.do?answer(TabPanelName)=Insured_Panel&customerId=<%= customerId.toString()%>&answer(Object::Risk::ResProperty::CustomerId)=<%=customerId.toString() %>&answer(Object::Customer::CustomerId)=<%= customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyIdTemp.toString()%>&answer(objectId)=0&answer(Object::Risk::ResProperty::ResPropertyId)=-1&answer(objectVerId)=0&answer(objectName)=HOME_RISK&answer(applicationType)=RISK&answer(applicationSubType)=&Navigation=&answer(teritory)=<%=""%>&answer(state)=<%=""%>&answer(territoryName)=<%=""%>&answer(zone)=<%=""%>&answer(showOnetTab)=Y&tabNo=0" title="Click to go to add a New Risk"/>Add Risk</a>				
				</td>
				...
				<logic:equal value="FALSE" name="paramValue">
					...
					<bean:define id="custProgramName" name="AddCustomer" property="answer(Object::CustomerAdditional::ProgramName)" scope="session" />
						<a href="../NewLifeAgentQuote.do?TabPanelName=Quote_Panel&answer(createQuoteEffectiveDate)=yes&answer(Object::Quote::primaryEntityId)=<%=ObjectId.toString()%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&formName=form::ADDQUOTEDETAILSLIFEAGENT&answer(Object::Quote::QuoteStatus)=In Progress&answer(Object::Quote::OwnerId)=<%=ownerId.toString()%>&answer(Object::Quote::CreatedBy)=<%=userId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyIdTemp.toString()%>&answer(Object::Quote::ProductStatus)=RELEASED&answer(State)=<%=EmpGrpState%>&answer(Object::Risk::RiskId)=1&answer(Object::Quote::NewQuote)=YES&answer(Object::CustomerAdditional::ProgramName)=<%= custProgramName%>" title="Click to go to create a New Quote">New Quote</a>
				</td>	
				</logic:equal>
		</logic:present>
			</tr>
			...
			<tr><td colspan="5">
				<table WIDTH="100%" ALIGN="left" cellspacing=1 cellpadding="1" border="0">
					...
					<TR >
						...
						<td class="sectionhead" align="center" >Effective Date</td>
						<td class="sectionhead" align="center" >Status</td>		
						<td class="sectionhead" align="center" >Quote Type</td>
						<td class="sectionhead" align="center" >Created Date</td>
...
<!-- Added to show subnumber along with quote number end -->
						...
						<TD class=<%= styleClass %> align="center"><bean:write name="aQuote" property="quoteStartDate" /></TD>
						...
						<TD class=<%= styleClass %> align="center"><bean:write name="aQuote" property="createdDate" /></TD>
						<TD class=<%= styleClass %> align="center"><bean:write name="aQuote" property="updatedDate" /></TD>
						<td class=<%= styleClass %> align="right"><dmshtml:CurrencyFormatTag digits="2" format='<%=""+grandTotal%>' /></td>	
						<td class=<%= styleClass %>>&nbsp;&nbsp;<a href="../CopyQuote.do?TabPanelName=Quote_Panel&quoteId=<%=""+quoteId%>&answer(Entity::Equals::OwnerId)=<%=ownerId1.toString()%>&parentQuoteVerId=<%=""+parentQuoteVerId.toString()%>&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyIdTemp.toString()%>&answer(Object::Customer::State)=<bean:write name='aQuote' property='customerState' />" />Copy<BR></a>
						</td>
						<% if(deletePrivilege){%>
