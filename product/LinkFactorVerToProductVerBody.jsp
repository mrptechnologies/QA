...
<html:form action="/LinkFactorVerToProduct">
...
%>
	...
	<table WIDTH="95%" ALIGN="center" BORDER="0" CELLSPACING="2"
		...
		<tr>
			<td class="MH">LINK FACTOR VERSIONS AND PRODUCT</td>
		</tr>
		<tr>
			<td class="TextMatter">Use this page to link factor versions to
			product <br>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="FormLabels" align="left">Select factor version(s) to
		...
		<tr>
			...
			<table WIDTH="550px" ALIGN="center" cellspacing=0 cellpadding="2"
				...
				<tr>
					<td class="FormLabels" align="left">Factor Versions</td>
					<td class="FormLabels" align="left">&nbsp;</td>
					<td class="FormLabels" align="left">Selected Factor Versions</td>
				...
				<bean:define id="ownerId"
					...
					scope="session" />
				<tr>
					<td width="15%"><dmshtml:get_available_factors_ver
					...
					<dmshtml:dms_select property="answer(Object::Product::Factor)"
						...
						<logic:present name="arrFactors" scope="request">
							<logic:iterate id="factors" name="arrFactors" scope="request">
								<bean:define id='factorVerName' name="factors"
									property="factorVerName" />
								<bean:define id='factorVerNum' name="factors"
									property="versionNumber" />
								<bean:define id='factorVerId' name="factors"
									property="factorVerId" />
								<dmshtml:dms_option value='<%=factorVerId.toString()%>'>
									<%=factorVerName.toString()+factorVerNum.toString() %>
								</dmshtml:dms_option>
							</logic:iterate>
						</logic:present>
					...
					<td width="30%"><dmshtml:get_product_ver_factors
					...
					<dmshtml:dms_select
						...
						<logic:present name="arrProdFactors" scope="request">
							<logic:iterate id="prodFactors" name="arrProdFactors"
								scope="request">
								<bean:define id='factorVerName' name="prodFactors"
									property="factorVerName" />
								<bean:define id='factorVerNum' name="prodFactors"
									property="versionNumber" />
								<bean:define id='factorVerId' name="prodFactors"
									property="factorVerId" />
								<dmshtml:dms_option value='<%=factorVerId.toString()%>'>
									<%=factorVerName.toString()+factorVerNum.toString() %>
	...
	<html:hidden property="answer(objectType)" value="productVerFactor" />
