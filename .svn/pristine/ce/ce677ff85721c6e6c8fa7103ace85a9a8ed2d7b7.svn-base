...
<bean:define id="submissionId" property="answer(submissionId)" name="AdditionalInsured" />
<bean:define id="submissionVerId" property="answer(submissionVerId)" name="AdditionalInsured" />
<html:form action="/SaveSubmissionAdditionalInsuredAction" onsubmit="return Validate();">
...
<html:hidden property="Object::Submission::SubmissionStatus" styleId="SubmissionStatus" value='<%=""+insuredStatus %>'/>
...
<dmshtml:GetParameterByOwnerId nameInSession="paramValue" ownerId="<%=ownerId.toString()%>" dbname='<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>' parameterName="CUSTOMER_NAME_LENGTH" />
	...
	<table WIDTH="90%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
			...
			<tr><td><a href="../GetInsuredAction.do?TabPanelName=Insured_Panel&customerId=<%=""+Customerid%>" class="button2" style="width:150px" >Go To Insured Summary</a></td></tr>
			<tr bordercolor="red">
		...
		<tr>
			...
			<td class="links">&nbsp;<html:text  name="AdditionalInsured" property ="answer(Object::AdditionalInsured::InsuredName)" size="65" maxlength="125" styleClass="txtbox" styleId="InsuredName" /></td>
		</tr>
		<tr id="DBABlock">
		...
		<tr>
			...
			<td>
				<dmshtml:dms_static_with_connector ruleName='privilage' property='<%=new String[]{"UNDERWRITER_PRIVILEGE"}%>' connector='or' negated="true">
					&nbsp;<html:text  name="AdditionalInsured" property ="answer(Object::AdditionalInsured::InsuredNumber)" size="8" maxlength="8" styleClass="txtbox" styleId="DBA" />
		...
		<tr>
			...
			<td class="links">
			&nbsp;<html:text  name="AdditionalInsured" property="answer(Object::AdditionalInsured::Zip)" size="10" maxlength="10" styleClass="txtbox" styleId="Zip" onkeyup="if(this.value.length==5) {this.value=this.value+'-'}"/>		
...
<html:hidden property="answer(submissionVerId)" value="<%=""+submissionVerId %>"/>      
<html:hidden property="answer(submissionId)" value="<%=""+submissionId %>"/>      
...
function callShowInsuredInformation(){
 ...
 for(index=0;index<document.forms[0].elements.length;index++){
 	...
 	var elemtType = document.forms[0].elements[index].type;
 	
 	if(document.getElementById('SubmissionStatus').value != 'In Progress'){
	 	if( elemtType == 'text'){
	 		document.forms[0].elements[index].readOnly = true;
	 	}else{
	 		document.forms[0].elements[index].disabled = true;
	 	}
 	}
 }
