 ...
 */
 ...
 activateQuoteTabs(ratingDetailId);	
	...
	var custId = document.getElementById("customerId").value;
 	...
 	var url = '../GetRatingDetailForm3.0.do?'+pars;	
...
var frm = "";
//if(document.forms[0].ID449 != undefined){
...
var daysInMonth = DaysArray(12);
		if(frm.ID449 !=null && frm.ID449 !=undefined){
			for(i=0;i<frm.ID449.length;i++){
				var vardate = frm.ID449[i].value;
				var elem = frm.ID449[i];
				if (vardate=="")
				{
			...
			if(frm.ID449.length == undefined){
			var vardate = frm.ID449.value;
				var elem = frm.ID449;
				if (vardate=="")
				{
					alert("Please enter the Retro Date");
			...
			if(frm.ID447 !=null && frm.ID447 !=undefined){
			
				for(i=0;i<frm.ID447.length;i++){
					var vardate = frm.ID447[i].value;
					var elem1 = frm.ID447[i];
					if (vardate=="")
					{
				...
				if(frm.ID447.length == undefined){
					var vardate = frm.ID447.value;
					var elem1 = frm.ID447;
					if (vardate=="")
						{
							alert("Please enter the Anniversary Date");
