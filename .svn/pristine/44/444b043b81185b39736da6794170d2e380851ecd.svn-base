...
<SCRIPT type="text/javascript" SRC="../Scripts/JScripts/QuoteSummary.js"> </SCRIPT>
		...
		<bean:parameter id="parentPolicyId" name="answer(parentPolicyId)"/>
		<bean:parameter id="parentPolicyVerId" name="answer(Object::Policy::ParentPolicyVerId)"/>
...
<html:form action="/CreateQuotePraposal.do" onsubmit="return checkActionSubmit(this);">
											...
											<td >
												<table width="60%" align="Left" border="0">
													<jsp:include page="../common/QuoteHeader.jsp">
														...
														<jsp:param name="heading" value="<%="CREATE RENEWAL PROPOSAL"%>" />
													</jsp:include>
												</table>
											</td>
										</tr>
												...
												<table WIDTH="100%" ALIGN="left" border="0" CELLSPACING="2" CELLPADDING="0">
													...
													<tr>
														...
														<td colspan="2" class="links" width="150px">
															<a href="../GetUWQuoteSummaryAction.do?TabPanelName=Quote_Panel&answer(Object::Customer::CustomerId)=<%=customerId.toString()%>&answer(Object::Agency::AgencyId)=<%=agencyIdTemp.toString()%>&answer(QuoteId)=<%=strParentQuoteId.toString()%>&answer(parentQuoteId)=<%=strParentQuoteId.toString()%>&answer(policyNumber)=<%=policyNumber.toString() %>&answer(Object::Policy::ParentPolicyId)=<%=parentPolicyId.toString() %>&answer(Object::Policy::ParentPolicyVerId)=<%=parentPolicyVerId.toString() %>">Quote Summary</a>
														</td>
...
<tr>
	...
	<dmshtml:dms_isPresent property='<%=new String[]{"In Progress"}%>' value="<%=paretQuoteStatus.toString()%>">
		<td class="TextMatter" align="right">
			Are you sure you want to create a Renewal proposal?
...
<!-- @ -->
	...
	<tr>
		<td>
			...
			<html:hidden property="answer(parentPolicyId)" value="<%=""+parentPolicyVerId%>" />
