...
<html:form action="/LinkStateToProductVer">
...
<tr><td>&nbsp;</td></tr>
<tr><td class="MH">
LINK STATES AND PRODUCT VERSIONS
</td></tr>
<tr><td class="TextMatter">
Use this page to link states to product versions
<br>
</td></tr>
...
<td class="FormLabels" align="left">Select state(s) to be linked to the product version</td></tr>
<tr><td>
<table WIDTH="550px" ALIGN="center" cellspacing=0 cellpadding="2" border="0">
	...
	<tr>
			...
			<td class="FormLabels" align="left">Selected States</td>
			
</tr>
<bean:define id="ownerId" property="answer(Object::UserDetail::ownerId)" name="LoginPage" scope="session"/>
<tr>
		<td width="15%"><dmshtml:get_available_prodVer_states nameInSession="arrStates" ownerId="<%=Long.parseLong(""+ownerId)%>"  productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>" 
productVerId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductVerId)").toString())%>" dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::Product::StateCode)" styleId="possible" multiple="true" size="7" >
           ...
           <logic:iterate id="states" name="arrStates" scope="request" >
		    <bean:define id='stateCode' name="states"  property="stateCode" />
		     <dmshtml:dms_option value ='<%=stateCode.toString()%>' >
		       <bean:write name="states" property="state"  />
		    </dmshtml:dms_option>
		</logic:iterate>
		</logic:present>
	</dmshtml:dms_select>	
...
<td width="30%"><dmshtml:get_prodVer_states nameInSession="arrStates" ownerId="<%=Long.parseLong(""+ownerId)%>" productVerId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductVerId)").toString())%>" dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::ProductVerState::StateCode)" styleId="chosen" multiple="true" size="7" >
			<logic:present name="arrStates" scope="request">
           <logic:iterate id="states" name="arrStates" scope="request" >
		    <bean:define id='state' name="states"  property="state" />
		     <dmshtml:dms_option value ='<%=state.toString()%>' >
		       <bean:write name="states" property="stateCode"  />
		    </dmshtml:dms_option>
	</logic:iterate>
		</logic:present>
	</dmshtml:dms_select>	
...
<html:hidden property="answer(objectType)" value="productVerState" />
