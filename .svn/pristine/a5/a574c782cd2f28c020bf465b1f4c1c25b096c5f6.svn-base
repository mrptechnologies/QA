...
<html:form action="linkEndorsementToProduct">
...
<tr><td>&nbsp;</td></tr>
<tr><td class="MH">
LINK ENDORSEMNT AND PRODUCTS
</td></tr>
<tr><td class="TextMatter">
Use this page to link a product to endorsements
<br>
</td></tr>
...
<td class="FormLabels" align="left">Select endorsement(s) to be linked to the product</td></tr>
<tr><td>
...
<tr>
		<td width="15%"><dmshtml:get_available_endorsement_prods nameInSession="arrProds" ownerId='<%=Long.parseLong(""+ownerId)%>' productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>" dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::Product::Application)" styleId="possible" multiple="true" size="7" >
           ...
           <logic:iterate id="prods" name="arrProds" scope="request" >
		    <bean:define id='productId' name="prods"  property="productId" />
		     <dmshtml:dms_option value ='<%=productId.toString()%>' >
		       <bean:write name="prods" property="MProductName"  />
		    </dmshtml:dms_option>
		</logic:iterate>
		</logic:present>
	</dmshtml:dms_select>	
...
<td width="30%"><dmshtml:get_product_endorsements nameInSession="arrProds" productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>" dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::Endorsement::ProductId)" styleId="chosen" multiple="true" size="7" >
			<logic:present name="arrProds" scope="request">
           <logic:iterate id="prods" name="arrProds" scope="request" >
		    <bean:define id='productId' name="prods"  property="productId" />
		     <dmshtml:dms_option value ='<%=productId.toString()%>' >
...
<html:hidden property="answer(objectType)" value="endorsement" />
