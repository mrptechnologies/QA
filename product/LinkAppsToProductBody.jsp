...
<html:form action="/linkAppToProduct">
...
<%if(request.getParameter("answer(appType)")!= null) {%>
	<html:hidden property="answer(appType)" value="<%=request.getParameter("answer(appType)").toString()%>" />
...
<tr><td>&nbsp;</td></tr>
<tr><td class="MH">
LINK APPLICATIONS AND PRODUCT
</td></tr>
<tr><td class="TextMatter">
Use this page to link applications to product
<br>
</td></tr>
...
<td class="FormLabels" align="left">Select application(s) to be linked to the product</td></tr>
<tr><td>
...
<tr>
		<td width="15%"><dmshtml:get_all_apps_by_type nameInSession="arrApps" ownerId="<%=Long.parseLong(""+ownerId)%>"  productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>" 
		type="<%=request.getParameter("answer(type)").toString()%>" 
dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::Product::Application)" styleId="possible" multiple="true" size="7" >
           ...
           <logic:iterate id="apps" name="arrApps" scope="request" >
		    <bean:define id='applicationId' name="apps"  property="applicationId" />
		     <dmshtml:dms_option value ='<%=applicationId.toString()%>' >
		       <bean:write name="apps" property="applicationName"  />
		    </dmshtml:dms_option>
		</logic:iterate>
		</logic:present>
	</dmshtml:dms_select>	
...
<td width="30%"><dmshtml:get_product_apps_by_type nameInSession="arrApps" ownerId="<%=Long.parseLong(""+ownerId)%>" productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>"
		type="<%=request.getParameter("answer(type)").toString()%>" dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::ProductApp::ApplicationId)" styleId="chosen" multiple="true" size="7" >
			<logic:present name="arrApps" scope="request">
           <logic:iterate id="apps" name="arrApps" scope="request" >
		    <bean:define id='applicationId' name="apps"  property="applicationId" />
		     <dmshtml:dms_option value ='<%=applicationId.toString()%>' >
...
<html:hidden property="answer(objectType)" value="Application" />
