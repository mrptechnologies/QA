...
<html:form action="/SearchZipCodeCommon.do"  styleId="SearchZipCodeCommon" onsubmit="checkForInvalidSessionCloseAndForward('N');return validateRequest(this)">
<html:hidden property="answer(dbname)" value='<%=com.dms.web.util.DbUtils.getWriteDS(pageContext)%>'/>
	<table WIDTH="95%" ALIGN="center" BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<tr>
			...
			<td><dmshtml:dms_text name="SearchZipCodeCommon"  property="answer(Object::Customer::County)" title="Please enter County to search" styleId="ThisCounty" size="20" maxlength="20" styleClass="txtbox"/></td>
		...
		<html:hidden property="answer(HomeFlag)" value="<%=""+HomeFlag%>" />
			...
			<html:form action="/SearchZipCodeCommon.do" styleId="getZipCodeDetail" onsubmit="if(submitToParent(this)) {checkForInvalidSessionCloseAndForward('Y');} else {return false;}">
		...
		<tr><td>&nbsp;</td></tr>
		 <tr>	
			<logic:present name="prevZip"  scope="request" >
				<td  class="links">
					<a href="../SearchZipCodeCommon.do?&FormName=<%=frmName%>&MultiplefrmWithSameName=<%=isMultipleFormWithSameName%>&FormID=<%=formID%>&HomeFlag=<%=""+HomeFlag%>&answer(Object::ZipCode::PageNum)=<%=(Integer.parseInt(""+pagenum)-1)%>&answer(ownerId)=<%=""+ownerId%>" onclick="checkForInvalidSessionCloseAndForward('N');"><< Previous</a>
				</td>
			</logic:present>

			<logic:present name="moreZip"  scope="request" >
				<td align="right" class="links" colspan="6">
					<a href="../SearchZipCodeCommon.do?&FormName=<%=frmName%>&MultiplefrmWithSameName=<%=isMultipleFormWithSameName%>&FormID=<%=formID%>&HomeFlag=<%=""+HomeFlag%>&answer(Object::ZipCode::PageNum)=<%=(Integer.parseInt(""+pagenum)+1)%>&answer(ownerId)=<%=""+ownerId%>" onclick="checkForInvalidSessionCloseAndForward('N');">Next >></a>
