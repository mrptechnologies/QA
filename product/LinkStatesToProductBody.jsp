...
<td class="FormLabels" align="left">Select state(s) to be linked to the product</td></tr>
<tr><td>
...
<tr>
		<td width="15%"><dmshtml:get_all_states nameInSession="arrStates" ownerId="<%=Long.parseLong(""+ownerId)%>"  productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>" dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
...
<td width="30%"><dmshtml:get_product_states nameInSession="arrStates" ownerId="<%=Long.parseLong(""+ownerId)%>" productId="<%=Long.parseLong(request.getParameter("answer(Object::Product::ProductId)").toString())%>" dbname="<%=com.dms.web.util.DbUtils.getReadDS(pageContext)%>"/>
		<dmshtml:dms_select  property="answer(Object::ProductState::StateCode)" styleId="chosen" multiple="true" size="7" >
			<logic:present name="arrStates" scope="request">
           <logic:iterate id="states" name="arrStates" scope="request" >
